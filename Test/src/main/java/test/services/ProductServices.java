...
public static List<CategoryResponce> responceByCategory(String category) {
	...
	//KOD NA SASKO STO NEMA SMISLA

	public static List<Product> getProductsByPrice(Integer categoryid, Product product) {
		Session session = init();
		Transaction tx = null;
		try {
			session = factory.openSession();
			tx = session.beginTransaction();
		//	Query query = session.createQuery("FROM main.java.test.entity.Product p WHERE p.categoryid=:categoryid");
		//	query.setParameter("categoryid", categoryid);
			Product product1 = session.get(Product.class, categoryid);
			List<Product> result = (List<Product>) product1.getCategory();
			for (Product product2 : result) {
				if(product2.getProductId() == product2.getCategory().getCategoryId()) {
					product2.getName();
					product2.getPrice();
					product2.getOrigin();
					...
					System.out.println("you dont have that kind of ID");
				}
			}
