...
import main.java.test.entity.ListProducts;
...
public static Integer createCart2(Integer id, ListProducts listProducts) {
		...
		try {
			...
			ShoppingCart cart = new ShoppingCart(listProducts.getCart().getProduct(),customer, date);
	...
	private static Customer getCustomerById(Integer id) {
		...
		try {
			...
			Query query = session.createQuery("FROM main.java.customer.entity.Customer c WHERE c.customerId=:customerId");
